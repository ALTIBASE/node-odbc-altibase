...
describe('Queries...', () => {
  ...
  it('...should pass w1nk/node-odbc issue #54', async () => {
    const result = await connection.query(`select cast(-1 as INTEGER) as TEST1, cast(-2147483647 as INTEGER) as TEST2, cast(2147483647 as INTEGER) as TEST3 ${global.dbms === 'ibmi' ? 'from sysibm.sysdummy1' : ''}`);
    assert.notDeepEqual(result, null);
    assert.deepEqual(result.length, 1);
    assert.deepEqual(result[0].TEST1, -1);
    assert.deepEqual(result[0].TEST2, -2147483647);
  ...
  // cast as CLOB is not possible in Altibase
  if (dbms !== 'altibase') {
